    ...
    return (
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-stone-100 max-w-lg mx-auto w-full animate-fade-in">
            <div className="flex items-center gap-4 mb-8 pb-4 border-b border-stone-100">
                ...
                <h2 className="text-2xl font-serif text-stone-800">Your Trends</h2>
            </div>

            <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-indigo-50 p-5 rounded-2xl border border-indigo-100 text-center">
                    <div className="text-3xl font-serif text-indigo-900 mb-1">{stats.total}</div>
                    <div className="text-xs font-bold text-indigo-400 uppercase tracking-widest">Sent</div>
                </div>
                <div className="bg-teal-50 p-5 rounded-2xl border border-teal-100 text-center">
                    <div className="text-3xl font-serif text-teal-900 mb-1">{stats.unlocked}</div>
                    <div className="text-xs font-bold text-teal-400 uppercase tracking-widest">Opened</div>
                </div>
            </div>

            <div>
                <h3 className="text-sm font-bold text-stone-400 uppercase tracking-wider mb-4">Mood History</h3>
                <div className="space-y-3">
                    {stats.topMoods.length === 0 ? (
                        <p className="text-stone-400 text-sm italic">No mood data yet.</p>
                    ) : (
                        stats.topMoods.map(([emoji, count]) => (
                            <div key={emoji} className="flex items-center gap-3">
                                ...
                                <div className="flex-grow h-3 bg-stone-100 rounded-full overflow-hidden">
                                    <div
                                        className="h-full bg-stone-800 opacity-60 rounded-full"
                                        style={{ width: `${(count / stats.total) * 100}%` }}
                                    ></div>
                                </div>
                                <span className="text-stone-500 font-medium w-6 text-right">{count}</span>
                            </div>
                        ))
                    )}
                </div>
